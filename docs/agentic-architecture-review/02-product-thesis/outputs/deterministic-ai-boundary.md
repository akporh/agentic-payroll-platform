...
| Final compliance decisions | Sound as stated, one confirmed viability gap, now resolved by decision | Track Y1 requires operator approval before applying a compliance-rule change — F-02-12 found no current mechanism exists to apply such a change other than a developer-authored migration. **Per D-02-04 (2026-07-12)**: statutory-rule change management is now scoped as its own deterministic platform/compliance capability, independent of Y1; Y1 is restricted to detection/comparison/proposal and must never author, execute, or deploy a migration itself |

**Caveat on eligibility enforcement**: the boundary is sound in the sense that AI never decides who is paid. But Stage 01 F-01-14/F-01-19/F-01-20 found the *reason* an employee is excluded is not always explicitly recorded by the deterministic system itself (it's inferable from status/enrollment/contract facts, not stated outright at exclusion time) — meaning a State Explainer agent narrating "why" is synthesizing an explanation from raw facts, not reading a ground-truth reason field. This is a legitimate and appropriate use of AI (see F-02-11) but is worth naming explicitly: the AI is not deciding eligibility, but it is the first thing in the system that will ever state an eligibility *reason* in words. **Per D-02-03 (2026-07-12)**: this applies only to *current-state* eligibility narratives (why is this employee excluded from the run in progress); a request to explain why an employee was excluded from a *past* run is historical explanation and is blocked under the same decision until F-01-27/F-01-29/F-01-38 close.
