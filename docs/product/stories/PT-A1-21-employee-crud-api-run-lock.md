# `PT-A1-21` — Employee CRUD API + D-ARCH-1 run-lock/backdating guard (B1, Sprint 17)
...
None upstream (the story file itself states "Depends on: Nothing"). It is a documented precondition for `PT-A1-22` (unified creation path reusing this same `employee_repo`) and `PT-A1-25` (the split Edit/Change-Grade UI that calls these endpoints).
