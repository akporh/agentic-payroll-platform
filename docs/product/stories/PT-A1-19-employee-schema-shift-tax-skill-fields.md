# `PT-A1-19` — Employee schema fields: shift_type, state_of_tax, skill_level (NEW-GAP4/13)
...
None recorded as a precondition for this story itself. It is a documented **precondition for** `PT-A1-20`'s sibling item O3 (shift gate in `rule_evaluator.py`, out of this batch) and for the deferred LTA anniversary trigger (O5, backlog, not part of this batch).
...
None specific to this story. Related unresolved items (`PH_OT` `is_pensionable` deferral, tracked separately as `PT-A1-02`/D-010) are out of this batch's scope and not carried here.
