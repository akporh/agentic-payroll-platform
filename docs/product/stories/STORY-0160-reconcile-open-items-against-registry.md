# `STORY-0160` — Reconcile open roadmap items against the story registry
...
owner of the traceability layer
...
- Each open roadmap item either matches an existing backlog row (`STORY-0150`–`0155`) or receives a newly allocated ID.
- Every `PLAN.md` entry shows its `story_ref`, with one documented exception: Phase 2 Agent Layer items are **not** allocated, because `CAP-12` is held at zero stories by design under D-023/OQ-6 and populating it would contradict a standing decision.
