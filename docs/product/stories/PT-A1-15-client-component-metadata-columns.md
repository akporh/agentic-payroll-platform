# `PT-A1-15` — `client_component_metadata` add `is_active` + `proration_strategy` (Track J blocker)
...
Not directly user-facing — this is the schema foundation the Track J post-onboarding configuration UI (`PT-A1-11`, statutory override edit/toggle) is built on.
...
None upstream. This is itself a blocking dependency **for** `PT-A1-11` (WC-10/11 edit/toggle UI) and `PT-A1-17` (extended `/configuration` GET) — both read/write the columns this migration adds.
