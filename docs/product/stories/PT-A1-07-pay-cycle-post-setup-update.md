# `PT-A1-07` — WC-1: Pay-cycle post-setup update endpoint
...
None additional beyond the Track J migration blocker `PT-A1-15` (`client_component_metadata` `is_active`/`proration_strategy` columns), which gates the whole Track J batch's ability to ship together, not this endpoint specifically (WC-1 touches `pay_cycle`, not `client_component_metadata`).
