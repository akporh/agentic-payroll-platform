# `PT-A1-17` — Extend `/configuration` GET with IDs/is_active/proration_strategy
...
Depends on `PT-A1-15` (the `client_component_metadata.is_active`/`proration_strategy` migration) for the columns it now returns in the component-overrides section.
