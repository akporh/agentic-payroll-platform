# `PT-A1-42` — Workspace timesheet configuration + attendance code seeding (TM-1, Sprint 16)
...
None upstream in this batch. It is a documented precondition for `PT-A1-41` (attendance code + policy CRUD, which reads/writes the `attendance_code_config`/`attendance_policy_config` rows this story seeds).
