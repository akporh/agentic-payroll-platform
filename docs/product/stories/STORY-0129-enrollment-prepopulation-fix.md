# `STORY-0129` — Enrollment pre-population normalisation fix (fixes EMP-REG-5)

**Origin code(s):** `PT-A1-38` · `EMP-REG-5`
...
- Commit `98f2100` ("feat(sprint-27+28+fix): smart native upload, error visibility, workspace activation CTA, idempotent bulk inputs", 2026-06-15) — this fix is bundled into a larger commit alongside Sprint 27/28 and the `STORY-0130` (was `PT-A1-39`) workspace-activation-CTA fix; not isolated to an EMP-REG-5-fix-only diff.
