...
| 2026-07-15 | dev-levy-rule-pct | All | **NOT TAKEN** | **Gap entry, written retrospectively 2026-07-29 — this is not a reading.** `/arch-council` ran on this sprint (`docs/sprints/dev-levy-rule-pct/state.md`, stage `arch-council: complete`) but never appended its Signal History row. No record of the signal readings survives: `dev-levy-rule-pct/architecture.md` is itself a post-compaction reconstruction and contains no signal content. The row is left as NOT TAKEN rather than backfilled GREEN — the sprint's substance (a cadence-gating + `PERCENTAGE_OF_BASIC` change internal to the existing rule model) would most likely have read GREEN like its neighbours, but *likely* is not *observed*, and an invented audit row is a worse defect than an admitted hole. Discovery and the gate that now prevents recurrence: [`2026-07-29-skill-frontmatter-and-evidence-gate-audit.md`](../diagnostics/2026-07-29-skill-frontmatter-and-evidence-gate-audit.md). |
