...
| **D6** | Stale committed `.claude/memory/` folder | **(b)** | Archived under a clearly dated name (`.claude/memory-archived-2026-07/`), not deleted. **MOOT (2026-07-13, Changeset 10 execution):** repository inspection (`git log --all -- .claude/memory/`) found this folder was never actually committed — it is covered by `.gitignore` ("Claude auto-memory (session-local, not for version control)"). D6's premise (a stale *committed* folder) does not hold. Per explicit user decision, the rename/archive action is skipped: no rename, no `.gitignore` change, no commit of memory content. Changeset 10 proceeded with the CLAUDE.md consolidation half only. |
