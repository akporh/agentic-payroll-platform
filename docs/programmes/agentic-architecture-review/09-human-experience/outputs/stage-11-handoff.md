...
| C13/C14 extend existing components (`NativeUploadFlow`, `ColumnMappingPanel`) rather than adding a wizard — components with **three** consuming pages (Employees, PayrollInputsBulkUpload, PayrollResults reconciliation upload — RC-1), so C13 changes carry a three-surface regression obligation | Lower marginal UI cost than a greenfield onboarding product; the flow's stage boundaries also provide the EG-001/EG-003 baseline timestamps for free if emitted as events from day one — instrument the baseline **before** C13 ships (Stage 04's recommendation, now costed at near-zero) |
