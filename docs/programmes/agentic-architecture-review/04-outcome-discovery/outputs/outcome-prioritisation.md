...
| C7 — input anomaly detection | **Pursue after platform prerequisite** (calibration approach decided 2026-07-13, D-04-01) | Approved as a layered combination — absolute thresholds at launch, period-on-period variance as a second layer, peer-pattern comparison deferred. No longer blocked on an unresolved calibration question; now blocked only on its stated platform prerequisite: the exception-resolution workflow, which C7 must not ship without (binding condition of D-04-01) |
