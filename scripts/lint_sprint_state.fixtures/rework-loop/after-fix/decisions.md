# Decisions — `rework-loop` (fixture, snapshot: after-fix)

Same append-only log as `after-rework` — `decisions.md` never shrinks; `DEC-rework-loop-03` remains the historical record of why `implementation` was reopened, even though it is `complete` again in this snapshot.
